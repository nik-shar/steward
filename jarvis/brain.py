...
stream. Everything above them speaks Jarvis's own types — so a Tau upgrade, a
...
from jarvis.identity.system import build_system_prompt, personal_identity
...
from jarvis.policy.consent import ConsentGate, Prompter
from jarvis.session import append_messages, load_messages, new_session_id, session_path
from jarvis.tools.registry import CapabilityPack, Mounted, mount
...
class BrainError(RuntimeError):
    """Raised when Jarvis cannot be assembled coherently."""
...
def build_consent_hook(gate: ConsentGate) -> BeforeToolCall:
    ...
    Tau's hook returns ``(blocked, reason)`` — **True means block**. Jarvis's gate
...
class Jarvis:
    """One runnable Jarvis: settings, memory, policy, packs, and the harness.
    ...
    @classmethod
    def from_env(cls, **kwargs: Any) -> Jarvis:
