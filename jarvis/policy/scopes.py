...
``INTERNAL``
    Writes to Jarvis's *own* stores — memory, the audit log. Audited, not gated.
    Nagging for consent on every remembered fact would make the system unusable,
    and the blast radius is Jarvis's own memory, which is append-only.
``WRITE``
    Affects the world outside Jarvis — a calendar, a file, a message. Consent
