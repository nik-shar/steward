...
2. **Personal** — anything in ``STEWARD_HOME/identity/*.md`` is private, never
...
def read_markdown(path: Path) -> str:
    ...
    Missing personal identity is normal, not an error — Steward should run from a
...
def _now_line(moment: datetime | None = None) -> str:
    ...
    Without this the model guesses the date, and a steward that does not know what
