...
DEFAULT_HOME = Path.home() / ".steward"
...
def load_env() -> None:
    """Load `.env` from the working tree, then from ``STEWARD_HOME``.
    ...
    home = Path(os.environ.get("STEWARD_HOME", str(DEFAULT_HOME))).expanduser()
...
class Paths:
    ...
    def from_env(cls) -> Paths:
        home = Path(os.environ.get("STEWARD_HOME", str(DEFAULT_HOME))).expanduser()
        vault = Path(os.environ.get("STEWARD_VAULT", str(home / "vault"))).expanduser()
        return cls(
            home=home,
            db=home / "steward.db",
...
class ProviderSettings:
    ...
    def from_env(cls) -> ProviderSettings:
        ...
        ``STEWARD_*`` is the canonical trio and wins when present. Otherwise the
        ...
        """
        if os.environ.get("STEWARD_API_KEY", "").strip():
            return cls(
                api_key=_env_text("STEWARD_API_KEY"),
                base_url=os.environ.get("STEWARD_BASE_URL", DEFAULT_BASE_URL).strip(),
                model=os.environ.get("STEWARD_MODEL", DEFAULT_MODEL).strip(),
                provider_name=os.environ.get("STEWARD_PROVIDER", "openai-compatible").strip(),
            )
        return cls(
...
def _parse_allowlist(raw: str | None) -> frozenset[str]:
    """Parse ``STEWARD_CONSENT_ALLOWLIST``.
...
class Settings:
    ...
    def tz(self) -> tzinfo:
        ...
        laptop, wrong for one on a server, so set `STEWARD_TIMEZONE` there. Every slot
        ...
        if not self.timezone:
            ...
            if local is None:  # pragma: no cover - only on an unconfigured system
                raise ConfigError("cannot determine the local timezone; set STEWARD_TIMEZONE")
            return local
        try:
            return ZoneInfo(self.timezone)
        except ZoneInfoNotFoundError as exc:
            raise ConfigError(
                f"STEWARD_TIMEZONE {self.timezone!r} is not a known IANA timezone "
    ...
    def load(cls) -> Settings:
        load_env()
        raw_turns = os.environ.get("STEWARD_MAX_TURNS", "12").strip()
        try:
            max_turns = int(raw_turns)
        except ValueError as exc:
            raise ConfigError(f"STEWARD_MAX_TURNS must be an integer, got {raw_turns!r}") from exc
        if max_turns < 1:
            raise ConfigError("STEWARD_MAX_TURNS must be at least 1")
        return cls(
            paths=Paths.from_env().ensure(),
            provider=ProviderSettings.from_env(),
            max_turns=max_turns,
            consent_allowlist=_parse_allowlist(os.environ.get("STEWARD_CONSENT_ALLOWLIST")),
            timezone=(os.environ.get("STEWARD_TIMEZONE") or "").strip() or None,
        )
